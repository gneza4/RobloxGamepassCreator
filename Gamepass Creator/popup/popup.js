...
async function removeAllGamepasses() {
  ...
  try {
    ...
    // Filter out gamepasses that are already off-sale (productId is null)
    const onsaleGamepasses = allGamepasses.filter(gp => gp.productId !== null);
    ...
    for (let i = 0; i < onsaleGamepasses.length; i++) {
      ...
      try {
        addLog(`Removing gamepass: ${gamepass.name} (ID: ${gamepass.id})...`, 'info');
        
        await takeGamepassOffSale(gamepass.id, state.csrfToken);
        addLog(`Removed: ${gamepass.name}`, 'success');
        
        successCount++;
        results.push({ 
          amount: gamepass.name, 
        ...
        addLog(`Failed to remove ${gamepass.name}: ${error.message}`, 'error');
        results.push({ 
          amount: gamepass.name, 
          success: false, 
          error: error.message,
